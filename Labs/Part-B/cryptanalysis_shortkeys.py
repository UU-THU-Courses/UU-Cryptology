...
def calculate_ic(input_text, valid_chars):
    """A function to compute index of coincidence (Ic)."""
    text_ic = 0
    n = len(input_text)
...
def perform_friedman_test(encrypted_text, candidate_key_lengths = None, min_keylength = 4, max_keylength = 16):
    ...
    if candidate_key_lengths is None or len(candidate_key_lengths) == 0:
        for m in range(min_keylength, max_keylength+1):
            subtext_chunks = [encrypted_text[j::m] for j in range(m)]
...
def perform_kasiski_test(encrypted_text, min_keylength, max_keylength):
    """A function to perform kasiski test to estimate key length."""
    
    # First find all the repreated substrings
    repeated_substrings = find_repeated_substrings(encrypted_text, min_size = 4, max_size = 24)
...
def analyze_vigenere_shortkey(encrypted_text = None, min_keylength = 4, max_keylength = 16):
    """A function to perform cryptanalysis of vignere cipher"""

    # First perform Kasiski test to get possible key length candidates
    candidate_key_lengths = perform_kasiski_test(encrypted_text=encrypted_text, min_keylength=min_keylength, max_keylength=max_keylength)
    ...
    chosen_key_length, _ = perform_friedman_test(encrypted_text, candidate_key_lengths=candidate_key_lengths, min_keylength = min_keylength, max_keylength = max_keylength)
    print(f"Chosen Key Length (Using Friedman test) = {chosen_key_length}")

    # Determine the key from given key length
    subtext_chunks = [encrypted_text[j::chosen_key_length] for j in range(chosen_key_length)]
    possible_key = []
    ...
    # Use possible key to decrypt the message and return the content
    return decrypt(encrypted_text, possible_key, char_list=valid_chars)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the file contain encrypted text.",
    )
    parser.add_argument(
        "--min_key",
        type=int,
        default=2,
        help="Minimum key size (default: 2)",
    )
    parser.add_argument(
        "--max_key",
        type=int,
        default=16,
        help="Minimum key size (default: 16)",
    )
    args = parser.parse_args()

    t1 = time.time()
    if os.path.isfile(args.crypto_path):
        print(f"Solving crypto file: {args.crypto_path}")
        
        # Read the contents of the input file
        ecrypted_text = open(args.crypto_path, "r", encoding="UTF-8").read()

        # Perform Cryptanalysis on the given Ciphertext
        decryted_text = analyze_vigenere_shortkey(ecrypted_text, min_keylength = args.min_key, max_keylength = args.max_key)

        # Output the decrypted text to console
        print(f"\nDecrypted Text: \n{decryted_text}\n")
